...
- [x] Update `ArrayToPoly()` to auto-detect max_degree from array size
- [x] Handle oversized arrays: [1,1,1,1,2] with implied degree 4 → auto-caps at array length - 1
- [x] Add optional max_degree parameter to ArrayToPoly for explicit capping
- [x] Update `PolyToArray()` to respect max_degree if set (or calculate from actual polynomial)
