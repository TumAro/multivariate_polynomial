...
// Constructors
Particle::Particle() : coefficient(1) {}

Particle::Particle(const Atom& atom) : coefficient(1) {
    addAtom(atom);
}

// Member function implementations
void Particle::addAtom(Atom atom) {
    this->coefficient *= atom.coeff;
    atoms.push_back(Atom(1, atom.var, atom.exp));
    mapVars();
}

void Particle::mapVars() {
    variables.clear();
...
float Particle::operator()(std::map<char, float> values) const {
...
Polynomial Particle::partialEval(std::map<char, float> values) const {
    Particle p;
...
std::vector<float> Particle::getExpVec() const {
...
bool Particle::isLexGreater(const Particle& p2) const {
...
Particle Particle::operator*(const Atom& a2) const {
    Particle p = *this;
    p.addAtom(a2);
    return p;
}

Particle Particle::operator*(float b) const {
    Particle p = *this;
    p.coefficient *= b;
    return p;
}
...
bool Particle::operator==(const Particle& p2) const {
    if (this->coefficient != p2.coefficient ||
